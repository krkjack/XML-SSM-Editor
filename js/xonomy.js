...
Xonomy.setMode = function (mode) {
	...
	if (mode == "nerd") $(".xonomy").not(".side-menu").removeClass("laic").addClass("nerd");
	if (mode == "laic") $(".xonomy").not(".side-menu").removeClass("nerd").addClass("laic");
}
...
Xonomy.renderElement = function (element) {
	...
	if (required)
		html += '<span class="name" title="' + title + '" onclick="Xonomy.click(\'' + htmlID + '\', \'openingTagName\');shakeElement($(this));">' + displayName + '</span>';
	else
		html += '<span class="name" title="' + title + '" onclick="Xonomy.click(\'' + htmlID + '\', \'openingTagName\');">' + displayName + '</span>';
	if (required)
		html += "<span class='attrRequired' title='This attribute is required!'>*</span>";
...
Xonomy.renderAttribute = function (attribute, optionalParentName) {
	...
	var onclick = '';
	if (required)
		onclick = ' onclick="Xonomy.click(\'' + htmlID + '\', \'attributeName\');shakeElement($(this))"';
	else 
...
Xonomy.showBubble = function ($anchor) {
	...
	function verticalPlacement() {
		...
		if (offset.top + height + bubbleHeight <= screenHeight) {
			...
			bottom = (screenHeight - offset.top) + "px";
...
SSM.showModal = function (dataname) {
	$("#modal".concat(dataname)).modal('show');
}

SSM.getModalAttributeValues = function (dataname) {
	var modalWindowID = "#modal" + dataname;
	$(modalWindowID).find($("input[id$='_input']",)).each(function () {
		var ssmModalAttrID = $(this).attr('id');
		var ssmModalAttrName = ssmModalAttrID.replace('_input', '');
		console.log(ssmModalAttrName);
		$(this).getVal();
	});
}

SSM.genNodeSSM = function (dataname) {
	//var currentModal = $("#modal".concat(dataname))
	//$(currentModal.modal('show');
	//$(currentModal).on('click', '#addElement', function(e) {
